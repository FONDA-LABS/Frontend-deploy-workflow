...
class HeroSwiper extends Base {
    ...
    #initSwiper() {
        if (document.querySelector('.text-swiper-hero') == null || document.querySelector('.image-swiper-hero') == null) {
            return;
        }
        ...
        const swiper1 = new Swiper('.text-swiper-hero', {
            slidesPerView: 1,
            spaceBetween: 40,
            loop: true,
        ...
        const swiper2 = new Swiper('.image-swiper-hero', {
            slidesPerView: 1,
            spaceBetween: 0,
            loop: true,
            speed: ANIMATION_DURATION,
        ...
        const swiperIndexWrapper = document.querySelector('.swiper-index-wrapper-hero');

        if (swiperIndexWrapper === null) {
            return;
        }

        const numberContainer = swiperIndexWrapper.querySelector('.number-container-hero');
        ...
        swiper2.on('slideChange', function () {
            ...
            updateNumber(newIndex, oldIndex);
        ...
        function updateNumber(newIndex, oldIndex) {
        ...
        swiper1.on('slideChangeTransitionEnd', function (swiper) {
            if (swiper.shouldChangeState === true) {
                ...
                if (auto_animation === false) {
                    let pausePosition = getCurrentCircleProgress();
                    ...
                    let pausePosition = getCurrentCircleProgress();
                    circle.style.transition = 'stroke-dashoffset 0s linear';
                    updateCircleProgress(pausePosition);
                }
                swiper1.shouldChangeState = false;
        ...
        function getCurrentCircleProgress() {
            const circle = document.getElementById('circle');
            const radius = circle.r.baseVal.value;
            const circumference = 2 * Math.PI * radius;
